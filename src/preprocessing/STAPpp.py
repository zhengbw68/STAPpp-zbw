...
class Node():
    ...
    def format(self):
        res = '%8d' % self.index
        # 6-field format (3 bcode) for nodes without rotation BCs
        # 9-field format (6 bcode) for nodes with rotation BCs
        has_rotation_bc = len(self.bounds) > 3 and any(self.bounds[i] != 0 for i in range(3, len(self.bounds)))
        nfields = 6 if has_rotation_bc else 3
        for i in range(nfields):
            res += ' %3d' % self.bounds[i]
