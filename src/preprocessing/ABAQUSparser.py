...
class Parser():
    ...
    def parseHeading(self):
        ...
        self.heading = self.getNextLine().strip()
    ...
    def parseMaterials(self):
        ...
        while True:
            ...
            if '*Material' in line:
                ...
                # Stop at first non-material line
                self.goBack()
                break
    ...
    def parseMaterial(self):
        ...
        material.name = re.match(
            r'\*Material, name=(\w+)', self.getNextLine()).groups()[0]
    ...
    def parseNset(self):
        ...
        while True:
            ...
            try:
                ...
                # ABAQUS keyword lines start with '*'
                if line.startswith('*'):
                    self.goBack()
                    break
                else:
        ...
        for bound in self.boundaries:
            for nset in bound.nsets:
                for nodeIndex in nset.nodeIndexs:
                    ...
                    b = list(node.bounds)
                    # Handle DOF range: displacementIndex to rotationIndex (both inclusive)
                    for dof in range(bound.displacementIndex - 1, bound.rotationIndex):
                        b[dof] = 1
                    node.bounds = tuple(b)
